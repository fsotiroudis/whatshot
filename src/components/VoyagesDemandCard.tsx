...
const VoyagesDemandCard: React.FC<VoyagesDemandCardProps> = ({ dayDate, vesselType, portName, onSignificantChange }) => {
  const [data, setData] = useState<VoyagesDemandRow[]>([]);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setError(null);
    fetch(`http://localhost:4000/api/voyages-demand?dayDate=${encodeURIComponent(dayDate)}&vesselType=${encodeURIComponent(vesselType)}&portName=${encodeURIComponent(portName)}`)
      .then(res => {
        ...
        <div>
          <div className="flex items-baseline">
            <span className="text-2xl font-semibold">
              {mostImportant.DEMAND}
            </span>
            <span className="ml-1 text-gray-500 text-sm">voyages</span>
          </div>
